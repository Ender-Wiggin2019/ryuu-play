...
describe('pokemon impl1 set_f', () => {
  ...
  it('allows 精神拥抱 to keep attaching to 飘飘球 when 勇气护符 increases HP', () => {
    const sim = TestUtils.createTestSimulator();
    const gardevoir = new GardevoirEx();
    const drifloon = new Drifloon();
    const charm = new BraveryCharm(getCard('勇气护符') as any);
    const psychicEnergies = TestUtils.makeEnergies([
      CardType.PSYCHIC,
      CardType.PSYCHIC,
      CardType.PSYCHIC
    ]);
    ...
    player.active.pokemons.cards = [gardevoir];
    player.bench[0].pokemons.cards = [drifloon];
    player.bench[0].trainers.cards = [charm];
    player.discard.cards = [...psychicEnergies];
    ...
    for (let i = 0; i < 3; i++) {
      ...
      sim.dispatch(new ResolvePromptAction(prompt.id, [
        {
          to: { player: PlayerType.BOTTOM_PLAYER, slot: SlotType.BENCH, index: 0 },
          card: player.discard.cards[0],
        },
      ]));
    }

    expect(player.bench[0].energies.cards.length).toBe(3);
    expect(player.bench[0].damage).toBe(60);
  });
