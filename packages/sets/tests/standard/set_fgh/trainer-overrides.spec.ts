...
import { BuddyBuddyPoffin } from '../../../src/standard/set_fgh/buddy-buddy-poffin';
import { CapturingAroma } from '../../../src/standard/set_fgh/capturing-aroma';
import { PokeBall } from '../../../src/standard/set_fgh/poke-ball';
...
describe('set_fgh trainer overrides', () => {
  it('replaces generated empty trainers with real card implementations', () => {
    const pokeBall = setFgh.find(card => card.name === '精灵球');
    const poffin = setFgh.find(card => card.name === '友好宝芬');
    const aroma = setFgh.find(card => card.name === '捕获香氛');
...
describe('BuddyBuddyPoffin set_fgh', () => {
  let sim: Simulator;
...
describe('CapturingAroma set_fgh', () => {
