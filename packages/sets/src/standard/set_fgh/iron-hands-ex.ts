...
export class IronHandsEx extends PokemonCard {
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof AttackEffect && effect.attack === this.attacks[1]) {
      effect.opponent.active.marker.addMarker(this.BONUS_PRIZE_MARKER, this);
      return state;
    }

    if (effect instanceof KnockOutEffect && effect.target.marker.hasMarker(this.BONUS_PRIZE_MARKER, this)) {
      effect.prizeCount += 1;
      effect.target.marker.removeMarker(this.BONUS_PRIZE_MARKER, this);
      return state;
    }

    if (effect instanceof EndTurnEffect) {
      const opponent = StateUtils.getOpponent(state, effect.player);
      opponent.active.marker.removeMarker(this.BONUS_PRIZE_MARKER, this);
      return state;
    }
