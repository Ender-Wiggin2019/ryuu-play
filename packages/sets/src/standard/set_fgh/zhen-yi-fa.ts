...
function isActivePokemonsAbility(effect: PowerEffect, owner: any, self: ZhenYiFa): boolean {
  if (effect.card === self) {
    return false;
  }

  const pokemonSlot = StateUtils.findPokemonSlot((effect as any).state ?? ({} as State), effect.card);
  return owner.active.getPokemonCard() === effect.card && pokemonSlot === owner.active;
}
